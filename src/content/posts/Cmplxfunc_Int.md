...
设$f(z)$在单连通区域D内连续，C为D 内任意一条简单封闭曲线，如果 $\oint_Cf(z)dz=0$,那么函数$f(z)$解析。

证明：可积说明存在原函数，$F^\prime(z)=f(z)$,并且原函数是解析函数，由解析函数可以无限阶求导以及求导之后解析性不变可以得到$f(z)$是解析函数

## 柯西不等式

设 $f(z)$ 在 $C:|z-z_0|=\rho$ 所围成的区域（注意这里是**闭圆盘**，包括圆周）上解析，若在圆周上有 $|f(z)|\leq M$ 则 $|f^{(n)}(z_0)|\leq\frac{n!M(\rho)}{\rho^n}$ 