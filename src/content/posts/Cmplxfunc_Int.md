...
平均值公式：设$D\sub\mathbb{C}$是一个开集，函数f在区域D上解析。闭圆盘$|z-z_0|=R$完全包含在解析区域内，那么函数f在圆心$z_0$处的值等于在圆周上的值的算数平均：
$$
...
用柯西积分公式进行证明
$$
f(z_0)=\frac{1}{2\pi}\oint\frac{f(z)}{z-z_0}dz\\
z=z_0+Re^{i\theta}\\
