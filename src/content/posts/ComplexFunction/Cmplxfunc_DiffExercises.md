...
由复变函数可导的充要条件：实部虚部函数可微且满足柯西黎曼条件： $\dfrac{\partial u}{\partial x}=\dfrac{\partial v}{\partial y}$,  $\dfrac{\partial u}{\partial y}=-\dfrac{\partial v}{\partial x}$
...
(3) $f(z)=\dfrac{x+y}{x^2+y^2}+i\dfrac{x-y}{x^2+y^2}$
