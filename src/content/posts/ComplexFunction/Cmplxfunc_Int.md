...
$$
\boxed{\int_Cf(z)dz=\int_{t_0}^Tf(z(t))z(t)^\prime dt}\\
...
$$
\boxed{\oint_Cf(z)dz=\oint_C{udx-vdy}+i\oint_C{vdx+udy}=0}
$$
...
$$
\boxed{\oint_C \frac{f(z)}{z-z_0}dz=f(z_0)2\pi i}
$$
...
$$
\boxed{f(z_0)=\frac{1}{2\pi}\int_0^{2\pi}f(z)d\theta}
$$
...
$$
\boxed{|f^{(n)}(z_0)|\leq\frac{n!M(\rho)}{\rho^n}}
$$
...
**morera定理**：设 $f(z)$ 在**单连通区域** $D$ 内连续，$C$ 为 $D$ 内任意一条简单封闭曲线，如果 $\oint_Cf(z)dz=0$,那么函数 $f(z)$ 解析。
