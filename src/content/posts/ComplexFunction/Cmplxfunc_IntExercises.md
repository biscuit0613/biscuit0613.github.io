...
>eg1: $\oint_C\dfrac{cos(\pi z)}{(z-1)^5}dz$ 其中C $|z|>1$
...
>eg2： $\oint_C\dfrac{e^z}{(z^2+1)^2}dz$ 其中C $|z|>1$

:::tip
把分母拆成 $(z-i)^2(z+i)^2$,根据不同的奇点选择不同的 $f(z)$ 和积分
路径，然后用高阶导数公式，再把结果加起来。
:::

$$
\begin{alignedat}{2}
&\qquad\oint_C\frac{e^z}{(z^2+1)^2}dz &&=\oint_C\frac{e^z}{(z-i)^2(z+i)^2}dz\\[5pt]
&\text{取 $f_1(z) =\dfrac{e^z}{(z+i)^2}$} &&\text{奇点 $z=i,n=1$}\\
&\qquad \qquad \qquad f_1^\prime(z) &&=\frac{e^z}{(z+i)^2}-\frac{2e^z}{(z+i)^3}\\
&\qquad\oint_C\frac{f_1(z)}{(z-i)^2}dz &&=2\pi i\frac{1}{1!}f_1^\prime(z=i)=2\pi i\frac{-e^i(1+i)}{4}=\frac{\pi e^i(1-i)}{2}\\[5pt]
&\text{取 $f_2(z)=\dfrac{e^z}{(z-i)^2}$}&&\text{奇点 $z=-i,n=1$}\\
&\qquad \qquad \qquad f_2^\prime(z) &&=\frac{e^z}{(z-i)^2}-\frac{2e^z}{(z-i)^3}\\
&\qquad\oint_C\frac{f_2(z)}{(z+i)^2}dz &&=2\pi i\frac{1}{1!}f_2^\prime(z=-i)=2\pi i\frac{-e^{-i}(1-i)}{4}=\frac{\pi e^{-i}(1+i)}{2}\\

\therefore&\qquad\oint_C\frac{e^z}{(z^2+1)^2}dz &&=\frac{\pi}{2}(e^i+e^{-i})+\frac{\pi i}{2}(e^{-i}-e^i)=\pi \cos 1-i\pi \sin 1
\end{alignedat}
$$

>eg3: $\oint_C\dfrac{z^3+1}{(z+1)^4}$

奇点 $z=-1,n=3$，取 $f(z)=z^3+1$，则
$$
f^{(3)}(z)=6\\
\oint_C\frac{z^3+1}{(z+1)^4}dz=2\pi i\frac{1}{3!}f^{(3)}(-1)=2\pi i \;\text{奇点在积分路径内}  
$$

>eg4 :$\oint_C\dfrac{dz}{(z^2+1)(z^2+4)}$ C: $|z|=\frac{3}{2}$
...
>eg5：$\oint_C\dfrac{3z^2+7z+1}{(z+1)^3}dz$ 其中C $|z+i|=1$

:::tip
奇点在 $z=-1$，在积分路径外，因此积分为0
:::

>eg6：$\oint_C\dfrac{3z+2}{z^4-1}dz$ 其中C $|z-(1+i)|=\sqrt{2}$
