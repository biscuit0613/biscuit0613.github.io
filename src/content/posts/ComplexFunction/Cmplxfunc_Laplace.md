...
| $\sin(bt)$        | $\dfrac{b}{s^2 + b^2}$ |$\dfrac{1}{s+1}=\mathcal{L}\{\sin(t)\}$|
| $\cos(bt)$        | $\dfrac{s}{s^2 + b^2}$     |$\dfrac{s}{s+1}=\mathcal{L}\{\cos(t)\}$|
...
两边同时取拉普拉斯变换，利用线性性质和微分性质，将微分方程转化为关于 $F(s)$ 的代数方程。
