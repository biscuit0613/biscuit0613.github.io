...
| 名称     | 记号        | 所在空间       | 维度  | 描述                                            |
| -------- | ----------- | -------------- | ----- | ----------------------------------------------- |
| 列空间   | $Col(A)$    | $\mathbb{R}^m$ | $r$   | A能产生的所有输出                               |
| 零空间   | $Null(A)$   | $\mathbb{R}^n$ | $n-r$ | 被压缩掉的方向，满足 $Ax=0$ 的所有解            |
| 行空间   | $Row(A)$    | $\mathbb{R}^n$ | $r$   | 输入中有效方向，由A的行向量张成的空间             |
| 左零空间 | $Null(A^T)$ | $\mathbb{R}^m$ | $m-r$ | 输出中无法达到的方向，满足 $A^T y=0$ |
...
矩阵的**初等变换**（就是行列操作，不改变矩阵的秩）也是一种线性映射，也可以由矩阵表示，可以归结为三种基本的行列操作：
...
参考[矩阵的初等变换](https://biscuit0613.github.io/posts/linearalgebra/linearalgebrabasicchange/)一文，了解初等矩阵的定义、性质和在矩阵运算中的应用。
...
=\{y | y = A\mathbf{x}, \mathbf{x} \in R^n\}
$$
...
=\{y | y = A^T\mathbf{x}, \mathbf{x} \in R^m\}
$$
...
这里是所有向量 $\mathbf{x}$ 的家。它被切割成了两个相互垂直（正交）的部分：

- 行空间 $C(A^T)$：含义：由矩阵 $A$ 的行向量组合而成的空间。

  - 物理意义：这是 $A$ 真正能“抓取”并变换的信息部分。如果一个向量落在行空间，它被 $A$ 乘以之后，一定会变成一个非零向量。
- 零空间 $N(A)$：含义：所有满足 $A\mathbf{x} = \mathbf{0}$ 的向量 $\mathbf{x}$。
  - 物理意义：这是 $A$ 的“黑洞”。任何落入这个空间的向量，经过 $A$ 变换后都会消失（变成零）。

- 关系：行空间与零空间是**正交补**。也就是说，任何输入向量 $\mathbf{x}$ 都可以唯一地拆解为一个“有用成分”（行空间）和一个“被抹除成分”（零空间）。

### 右侧：输出空间 $\mathbb{R}^m$ （$m$ 维空间）

这里是变换后的结果 $\mathbf{b}$（即 $A\mathbf{x}$）所在的家。它同样被切割为两部分

- 列空间 $C(A)$：含义：由矩阵 $A$ 的列向量组合而成。
  - 物理意义：方程 $A\mathbf{x} = \mathbf{b}$ 有解的充要条件是 $\mathbf{b}$ 落在列空间里。
- 左零空间 $N(A^T)$：含义：所有满足 $A^T\mathbf{y} = \mathbf{0}$ 的向量 $\mathbf{y}$。
