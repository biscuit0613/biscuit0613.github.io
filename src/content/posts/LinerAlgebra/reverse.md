...
在机器学习中，常常需要把数据 $D$（长方形 $n \times d$ 矩阵）求内积 $D^T D$ 得到方阵 $C$，然后求逆。如果多了一个样本维度（也就是有新数据 $\mathbf{v}$），D 就变成了 $(n+1) \times d$ 的矩阵，求内积得到的方阵 $C$ 就可以更新成 $D^TD+\mathbf{v}\mathbf{v}^T$ 能不能快速求逆呢？答案是可以的，利用矩阵逆引理：
...
这里面的向量 $\mathbf{u}$ 和 $\mathbf{v}$ 可以推广到扁平矩阵 $U$ 和 $V$，得到更一般的矩阵逆引理：
