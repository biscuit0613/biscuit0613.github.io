...
设 $\mathbf{x}$ 是 $A\mathbf{x} = \mathbf{b}$ 的一个精确解（难以得到），$\tilde{\mathbf{x}}$ 是数值计算得到的近似解，误差 $\mathbf{e}$ 和残差 $\mathbf{r}$ 定义如下，消去其中精确解的部分得到关系：
...
在实际计算中，单纯用误差和残差不能反映矩阵本身的稳定性，因此对绝对误差的上限的范数进行表示：
...
这说明相对误差的上限是条件数 $\kappa(A)$ 和相对残差 $\frac{||r||}{||\mathbf{b}||}$ 的乘积。因此，矩阵的条件数越大，即使残差很小，误差也可能很大，这就是矩阵不稳定的表现。
