...
其中 $L$ 是一个下三角矩阵，$U$ 是一个上三角矩阵。通过这种分解，我们可以将求解 $A\mathbf{x} = \mathbf{b}$ 转化为两个简单的三角矩阵方程：
...
手算步骤我们在对 $A$ 进行高斯消元得到 $U$ 的过程中，将消元所用的系数填入 $L$ 的对应位置。

第一步：处理第一列我们要把第 2 行和第 3 行的第一列元素化为 0。
