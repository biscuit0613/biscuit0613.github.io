...
$$
\begin{aligned}
R(\omega_1 | \mathbf{x}) &=  \lambda_{21} P(\omega_2 | \mathbf{x})=\lambda_{21}P(\mathbf{x}|\omega_1)P(\omega_1) \\
R(\omega_2 | \mathbf{x}) &= \lambda_{12} P(\omega_1 | \mathbf{x})=\lambda_{12}P(\mathbf{x}|\omega_2)P(\omega_2)
\end{aligned}
$$
...
$$
\hat{\omega} = \begin{cases}
\omega_1, & \text{if } \dfrac{P(\mathbf{x} | \omega_1) }{P(\mathbf{x} | \omega_2) } > \dfrac{P(\omega_2)\lambda_{12}}{P(\omega_1)\lambda_{21}} \\
\omega_2, & \text{otherwise}
\end{cases}
$$
...
二分类问题中，固定第一类错误率（$\epsilon_1$ , $\omega_1$ 被误判为 $\omega_2$）不超过某个阈值 $\alpha$，同时最小化第二类错误率（$\epsilon_2$ , $\omega_2$ 被误判为 $\omega_1$）。定义两类错误率：
