...
- 每个卷积核提取一种特征，生成一个 **特征图** （通道）。 $K$ 个卷积核就生成$K$个 **特征图**，形成一个 $K$ **通道** 的输出。
- 结果天然是一个高维 **张量** ：H(图像高度) x W(图像宽度) x K(通道数)
...
- **平均池化（Average Pooling）**：取窗口内的平均值，平滑特征图，减少噪声。

![alt text](image-2.png)

池化的作用：

1. **降维**：减少特征图的空间尺寸，降低计算量和内存使用。
2. 变相地增加卷积核的感受野，使模型能够捕捉更大范围的特征。
3. 其次是降低模型的复杂度。能够在一定程度上控制过拟合，提高分类器的泛 化能力
...
标准二维卷积（无分组、无膨胀、无深度可分离等变体）的参数量计算公式：
