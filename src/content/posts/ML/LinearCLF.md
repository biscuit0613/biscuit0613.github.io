...
对于两类 $y_i\in\{-1,1|i=1,2\}$ 训练数据:$y_1\{\mathbf{x}_1, \mathbf{x}_2, \ldots, \mathbf{x}_{n_1}\}$, $y_2\{\mathbf{x}_{n_1+1}, \mathbf{x}_{n_1+2}, \ldots, \mathbf{x}_{n_1+n_2}\}$
...
写成矩阵形式，把两类训练数据合到一个矩阵中,并且纳入偏置项1,称为**增广矩阵**，后文的推导中均以增广矩阵为基础：
...
\vdots & \vdots \\
\mathbf{x}_{n_1}^T & 1 \\
-\mathbf{x}_{n_1+1}^T & -1 \\
-\mathbf{x}_{n_1+2}^T & -1 \\
\vdots & \vdots \\
-\mathbf{x}_{n_1+n_2}^T & -1 \\
\end{bmatrix}
\begin{bmatrix}
w_1 \\
w_2 \\
...
这个解不唯一，定义一个**准则函数** $J(\mathbf{w})$，当 $\mathbf{w}$ 是解向量时，$J(\mathbf{w})$ 为最小；
...
定义标签 $y_i\in\{1,-1|i=1,2\}$ ，定义输入d维特征向量 $\mathbf{x}=(x_1, x_2, \ldots, x_d)^T$，增广特征向量 $\mathbf{x}=(x_1, x_2, \ldots, x_d, 1)^T$，权重向量 $\mathbf{w}=(w_1, w_2, \ldots, w_d, b)^T$。

**决策函数**（x经过增广处理）：$g(\mathbf{x}) = \mathbf{w}^T \mathbf{x}$

- 一个样本 $\mathbf{x}_i$ 到决策面的 **有符号距离** 为 $\dfrac{g(\mathbf{x}_i)}{\|\mathbf{w}\|}$，其中 $\|\mathbf{w}\|$ 是权重向量的范数，忽略。符号表示点位于哪一侧，大小表示离平面多远。
- 分类正确：$y_i g(\mathbf{x}_i) > 0$ 即真实标签与预测值同号。
- 分类错误：$y_i g(\mathbf{x}_i) < 0$ 即真实标签与预测值异号。

**准则函数**（批量下降）：设**错分类**的样本集合为 $\mathcal{X}$

$$
J_p(\mathbf{w})=\sum_{x\in\mathcal{X}} -(y_i g(\mathbf{x}_i))=\sum_{x\in \mathcal{X}} -\mathbf{w}^T \mathbf{x}_i=\sum_{x\in \mathcal{X}} -\mathbf{x}_i^T \mathbf{w}\\[1ex]
...
这里取符号转化为最小化优化问题，$y_i$ 是确定的常量，可以直接去掉。
