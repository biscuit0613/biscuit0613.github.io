...
- [二维连续型随机变量](#二维连续型随机变量)
  - [联合概率密度](#联合概率密度)
  - [边缘概率密度](#边缘概率密度)
  - [联合概率密度与联合分布函数的关系](#联合概率密度与联合分布函数的关系)
- [常见连续性二维随机变量概率分布](#常见连续性二维随机变量概率分布)
  - [二维均匀分布](#二维均匀分布)
  - [二维正态分布](#二维正态分布)
- [二维随机变量中各个变量的独立性](#二维随机变量中各个变量的独立性)
  - [分布函数判别法](#分布函数判别法)
  - [离散型：分布列判别法](#离散型分布列判别法)
  - [连续性：概率密度判别法](#连续性概率密度判别法)
...
- 联合分布函数对每一个变量均单调不减
- $\lim_\limits{x\to-\infin}F(x,y)=0$,$\lim_\limits{y\to-\infin}F(x,y)=0$,$\lim_\limits{x\to-\infin,y\to-\infin}F(x,y)=0$,$\lim_\limits{x\to+\infin,y\to+\infin}F(x,y)=1$
...
二维随机变量 $(X,Y)$ 中 $X,Y$ **各自的概率密度函数**称为**边缘概率密度**，记作 $f_X(x),f_Y(y)$ 由 $X,Y$ 各自的分布函数也就是**边缘分布函数**推导而来
$$
...
独立性定义：设 $F(x,y)$ 是二维随机变量 $(X,Y)$ 的联合分布函数，$F_X(x),F_Y(y)$ 分别是 $X$，$Y$ 的**边缘分布函数**（就是各自的分布函数），如果
$$
...
对于**离散型随机变量**，$X,Y$ 的**边缘分布列**(就是各自的分布列)分别是 $p_{i\cdot },p_{\cdot j}$，如果二维随机变量 $(X,Y)$ 的分布列 $p_{ij}$ 满足
$$
...
对于**连续型随机变量**，$X,Y$ 的**边缘概率密度函数**(就是各自的概率密度)分别是 $f_X(x),f_Y(y)$，如果二维随机变量 $(X,Y)$ 的联合概率密度函数满足
$$
\forall x,y\in\R,f(x,y)=f_X(x)\cdot f_Y(y)
$$
则称随机变量 $X,Y$ 相互独立
