...
- 联合分布函数对每一个变量均单调不减
- $\lim_{x\to-\infin}F(x,y)=0$,$\lim_{y\to-\infin}F(x,y)=0$,$\lim_{x\to-\infin,y\to-\infin}F(x,y)=0$,$\lim_{x\to+\infin,y\to+\infin}F(x,y)=1$
...
二维随机变量 $(X,Y)$ 中 $X,Y$ 各自的**概率密度函数**称为**边缘概率密度**，记作 $f_X(x),f_Y(y)$ 由 $X,Y$ 各自的分布函数也就是**边缘分布函数**推导而来
$$
