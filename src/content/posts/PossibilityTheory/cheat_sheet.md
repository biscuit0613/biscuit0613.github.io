...
$$
P(\text{至少发生一次}) = 1 - P(\text{一次也不发生}) \\[1em]
$$

至少一个不发生：

$$
P(\text{至少一个不发生}) = 1 - P(\text{都发生})
$$
...
当在**总体服从正态分布**的前提下，样本方差 $S^2$ 和样本均值 $\overline{X}$ 是独立的。否则这俩不一定独立（参考综合11选择5）证明方法超纲，这里不介绍
...
$$
(x_1, x_2, \ldots, x_n) \sim \prod_{i=1}^{n} f(x_i; \theta)
$$
