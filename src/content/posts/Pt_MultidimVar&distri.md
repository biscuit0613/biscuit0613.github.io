...
$$
为$X，Y$的**联合累计分布函数**，简称**联合分布函数**,即$(X\leq x)\cap (Y\leq y)$的概率，即图中阴影部分
...
3. 联合概率密度的**几何意义**：二重积分的几何意义是曲顶柱体的体积；设 $G$ 是平面 $xoy$ 的一个区域，则**点落在区域 $G$** 里面的**概率**为以 $G$ 为下底，$f(x,y)$ 为上底的曲顶柱体的体积
...
独立性定义：设 $F(x,y)$ 是二维随机变量 $(X,Y)$ 的联合分布函数，$F_X(x),F_Y(y)$ 分别是 $X$，$Y$ 的分布函数，如果
$$
