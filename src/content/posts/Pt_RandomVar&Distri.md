...
随机试验的样本空间$S$，对于$S$中每个样本点$e$，都有唯一的实数值$X(e)$与之对应，称$X(e)$为随机变量随机变量按取值方式可分为两类：离散型和连续型。
...
背景：假定我们有一系列伯努利试验，其中每一个的成功概率为p，失败概率为1-p。在获得**第一次成功**前要进行多次试验？
...
总体N很大，抽样n很小时，可以用**二项分布近似超几何分布**。此时次品率占主导地位，不放回的那一部分样品相比于总样品微不足道。

## 连续型随机变量及其概率密度

对于非离散型的随机变量X，其取值不能一一列举出来，因此就不能像离散型随机变量那样使用分布列描述它。
...
若连续性随机变量$X\sim N(\mu=0,\sigma^2=1)$，则称X服从标准正态分布。
