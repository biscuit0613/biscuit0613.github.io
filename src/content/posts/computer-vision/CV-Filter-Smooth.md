...
description: '线性平滑：均值滤波/高斯滤波（σ与核尺寸、可分离加速）；非线性平滑：中值/最大/最小滤波；双边滤波：Domain Kernel与Range Kernel联合加权、边缘保持特性'
image: ''
tags: []
category: '08-计算机视觉'
order: 4
...
$\sigma_d$ 和 $\sigma_r$ 分别控制"多大范围的邻居被纳入"和"多大差异的邻居被纳入"。实际调参时可以记住几条规律：

- $\sigma_d$ 固定时，$\sigma_r$ 过小 → 像素值差异被严格限制，边缘保留极好但平滑效果很弱（只有颜色完全一致的区域被平均）；$\sigma_r$ 过大 → 像素值核趋近于 1，双边滤波退化为高斯滤波。
- $\sigma_r$ 固定时，$\sigma_d$ 过小 → 只有几个像素被平均，去噪不够；$\sigma_d$ 过大 → 远处的像素即使灰度值差异大也会被纳入（$\sigma_r$ 控制跨边缘能力，但 $\sigma_d$ 过大还是会模糊细节）。
- 实践经验：$\sigma_r$ 设为图像灰度方差的 10%–20% 作为初始值，$\sigma_d$ 设为 2–5 像素。
