...
`forward` 先完成投影`proj`，例如q_proj 输出的 768 维（内存里是连续的一条），再用 `view` 把最后一个总维度拆成“头数 × 头维度”`view` 不会再学习一次投影，也不会把信息分发给独立的小网络；它只是重新解释连续内存中的维度，768 个连续的数,前 96 个算头 0,接下来 96 个算头 1,以此类推。。真正决定每个头看到什么子空间的是 $W_Q$、$W_K$ 和 $W_V$ 中不同的行，$W_Q$ 的第 0~95 行产生头 0 的 query,第 96~191 行产生头 1 的。：
...
MiniMind 默认 $h_qd_h=d$，所以 Query 投影（`q_proj`）前后的总维度相同；但这不等于没有发生变换。它仍然用一个 $d\times d$ 参数矩阵把每个 Token 的隐藏表示映射到了新的 Query 空间。K/V 总维度只有：
...
```text
  ...
  Q:  Q0 Q1 Q2 Q3 Q4 Q5 Q6 Q7   Q:  Q0 Q1 | Q2 Q3 | Q4 Q5 | Q6 Q7
       │  │  │  │  │  │  │  │        └─┬─┘   └─┬─┘   └─┬─┘   └─┬─┘
  K:  K0 K1 K2 K3 K4 K5 K6 K7   K:    K0      K1      K2      K3
  V:  V0 V1 V2 V3 V4 V5 V6 V7   V:    V0      V1      V2      V3

  8 组 K/V                         4 组 K/V，每组服务 2 个 Q 头
```

代码里的 n_rep 就是这个"每组服务几个":

```python
self.n_rep = self.n_local_heads // self.n_local_kv_heads   # 8 // 4 = 2
```

GQA 是 MHA 和 MQA 之间的插值:

| ~   | K/V头数量    | 说明                        |
| --- | ------------ | --------------------------- |
| MHA | $h_kv=h_q$   | 每个 Query 头都有独立的 K/V |
| MQA | $h_kv=1$     | 所有 Query 头共享同一组 K/V |
| GQA | $1<h_kv<h_q$ | 每组 K/V 服务多组 Query 头  |
