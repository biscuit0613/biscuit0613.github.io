...
本文暂不重复推导 Transformer 的 Attention，而是把重点放在**语言建模目标**上。
...
链式法则自然导出自回归建模：“自回归”表示当前变量的分布依赖于已经生成的历史变量。
...
如果预测 $x_t$ 时能够看到 $x_t$ 或更晚的 Token，模型就可能直接读取答案，训练目标也就失去了意义。这里先建立因果约束的概念；掩码矩阵、广播方式和 Decoder-only Block 的实现将在后续 Transformer 文章中展开。
