...
从上文可知，估计分为先验估计 $\mathbf{\hat{x}}_{k|k-1}$ 和后验估计 $\mathbf{\hat{x}}_{k|k}$ ，这两个估计的"真实值"都是**真实状态向量** $\mathbf{x}_k$ 。$\mathbf{P}_{k|k-1}$先验协方差矩阵描述先验估计（预测）的精度，$\mathbf{P}_{k|k}$后验协方差矩阵描述后验估计（更新）的精度。
