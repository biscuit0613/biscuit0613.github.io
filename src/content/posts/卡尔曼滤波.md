...
$\mathbf{x}_k$：**真实状态空间**：系统在时刻$k$的真实状态，无法观测（收到噪声影响）维度$n\times1$。

$\mathbf{\hat{x}}_{k|k-1}$：**先验状态/估计**：在$k$时刻使用$k-1$时刻的后验估计和系统真实的状态转移矩阵得到的预测，叫先验估计，维度$n\times1$。
...
&= \mathbf{F}_k \, \mathbb{E}\!\left[ \mathbf{e}_{k-1|k-1}\mathbf{e}_{k-1|k-1}^\top \right] \mathbf{F}_k^\top 
...
&= \mathbf{P}_{k|k-1}
   ...
   - \mathbf{P}_{k|k-1}\mathbf{H}_k^\top\mathbf{K}_k^\top \\
&\quad + \mathbf{K}_k\mathbf{H}_k\mathbf{P}_{k|k-1}\mathbf{H}_k^\top\mathbf{K}_k^\top
...
上面说卡尔曼矩阵描述了估计值的估计精度，所以也能决定卡尔曼增益$\mathbf{K}_k$的大小。
