...
求 $\mathbf{\hat{x}}_{k|k-1}$，根据 $k-1$ 时刻的**后验估计** $\mathbf{\hat{x}}_{k-1|k-1}$ 和系统的**状态传播模型**来预测 $k$ 时刻的状态：
$$
...
flowchart TD
   A[预测阶段] --> B[预测状态: $\hat{\mathbf{x}}_{k|k-1} = \mathbf{F}_k \hat{\mathbf{x}}_{k-1|k-1} + \mathbf{B}_k \mathbf{u}_k$]
   B --> C[预测协方差: $\mathbf{P}_{k|k-1} = \mathbf{F}_k \mathbf{P}_{k-1|k-1} \mathbf{F}_k^\top + \mathbf{Q}_k$]

   C --> D[更新阶段]
   D --> E[计算创新: $\mathbf{y}_k = \mathbf{z}_k - \mathbf{H}_k \hat{\mathbf{x}}_{k|k-1}$]
   E --> F[计算卡尔曼增益: $\mathbf{K}_k = \mathbf{P}_{k|k-1} \mathbf{H}_k^\top \left( \mathbf{H}_k \mathbf{P}_{k|k-1} \mathbf{H}_k^\top + \mathbf{R}_k \right)^{-1}$]
   F --> G[更新状态估计: $\hat{\mathbf{x}}_{k|k} = \hat{\mathbf{x}}_{k|k-1} + \mathbf{K}_k \mathbf{y}_k$]
   G --> H[更新协方差: $\mathbf{P}_{k|k} = (\mathbf{I} - \mathbf{K}_k \mathbf{H}_k) \mathbf{P}_{k|k-1}$]
