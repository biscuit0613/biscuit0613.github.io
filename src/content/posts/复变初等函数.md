...
2. $e^z$在$C$上**解析**且在$C$上$\frac{de^z}{dz}=e^z$
...
1. 运算性质：
   ...
   $$
2. 连续性（对于主值）
   $\ln z=\ln|z|+iargz$，在**去除原点和复实轴**的平面内连续

3. 解析性（对于主值）$\ln z=\ln|z|+iargz$，$\omega=\ln z$是$e^z$的反函数，结合反函数求导法则，$\ln z$**去除原点和复实轴**的平面内解析，且
   $$
...
1. 四则运算类比指数函数
2. 连续性：幂函数是由$e^\omega$ （处处解析）和$\omega=b\ Lnz$（去掉原点和复实轴解析）复合而成。所以幂函数在**去掉原点和复实轴**的平面内连续且解析
3. 求导：
   $$
   \begin{align*}
