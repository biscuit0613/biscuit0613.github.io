...
+ 曲线C上连续不代表曲线C上处处连续，曲线C上连续只是说C上的点**沿C的方向**是连续的，而处处连续要考虑**曲线外的方向**

### 复变函数连续性的性质

**连续性判定**： $z_0=x_0+iy_0,f(z)=u(x,y)+iv(x,y),f(z)$在$z_0$点连续的**充分必要**条件是$u(x,y),v(x,y)$在$(x_0,y_0)$均连续
  
1. 四则运算保持连续性：如果$f(z)$和$g(z)$在$z_0$点均连续，那么
   1. $f(z)\pm g(z)$
   2. $f(z)\cdot g(z)$
   3. $\frac{f(z)}{g(z)}(g(z)\neq0)$  
...
2. **复合保持连续性**：$h=g(z)$在$z_0$处连续，$\omega=f(h)$在$h=h_0=g(z_0)$时连续，那么复合函数$\omega=f(g(z))$在$z_0$处连续
