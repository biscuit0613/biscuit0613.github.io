...
#show: ilm.with(
  ...
  authors: "Lecture 02 点操作与空间增强  ·  Lecture 03 图像滤波与边缘检测",
...
#tip[
  #strong[考试对比小贴士：]
  - #strong[对数变换：]专门用于解决"动态范围太大，亮部和暗部无法同时看清"的问题。
  - #strong[伽马变换：]专门用于调控"图像整体过暗或过亮"，以及解决"显示器显示偏色"问题。
]
...
#formula[
  ...
  $ g(x, y) = (d-c)/(b-a) dot (f(x, y) - a) + c $
]

- #strong[应用场景：]解决因曝光不足或过度造成的#strong[图像灰度范围过窄（低对比度）]问题。
- #strong[分段线性灰度变换：]将整个灰度区间分割为多段，每段设不同斜率。相对于连续的对数或幂律变换，分段变换#strong[更为机械、僵硬，但参数精密可控]，适用于输出给计算机进行后续图像识别的场景。
...
- #strong[对比度诊断：]直方图集中在低灰度区（偏暗），集中在高灰度区（偏亮），或集中在极窄区间（细节模糊、对比度低）。理想图像灰度直方图分布均匀、宽广。
...
#strong[核心思想：]对图像分块（如 $k$ 个块），对每个块单独做直方图均衡化，但加入#strong[对比度限幅]。

#strong[计算流程：]

+ 步骤 1：将图像分割为若干个#strong[不重叠的块]。
+ 步骤 2：对每个块统计灰度直方图 $H(i)$，设定#strong[裁剪阈值 ClipLimit]。
+ 步骤 3：#strong[直方图裁剪：]凡超过 ClipLimit 的灰度级，强行将直方图顶部削平，裁剪下来的多余像素#strong[均匀重新分配给当前块的所有灰度级]。
+ 步骤 4：计算裁剪修改后的新直方图的 CDF，得到该块的转换函数。
+ 步骤 5：遍历整幅图像，对每个像素找到包围它的#strong[4 个相邻块的 CDF 函数]，分别查表得 4 个候选输出灰度值。根据像素在块间的位置做#strong[双线性插值]，加权平均得最终输出。
...
- #strong[NOT（非）：]$g(x, y) = 255 - f(x, y)$。获取#strong[阴图像]或子图像补图（等同于灰度反转变换）。
...
// ================================================================
// 五、Lecture 03：图像滤波与边缘检测 — 滤波基础概念与定义
// ================================================================

= 五、图像滤波基础概念与定义（Lecture 03）
...
- 高斯低通（GLPF）：$H(u,v) = e^(-D^2(u,v)/(2D_0^2))$，无振铃，最常用。
...
- #strong[梯度定义：]$nabla f = mat((partial f)/(partial x); (partial f)/(partial y))$。
- #strong[边缘强度：]$E_s = norm(nabla f) approx sqrt(((partial f)/(partial x))^2 + ((partial f)/(partial y))^2)$。工程上为简化计算，常使用绝对值近似：$E_s approx |I_x| + |I_y|$。
- #strong[边缘方向：]$theta = arctan(((partial f)/(partial y))/((partial f)/(partial x))) + pi/2$。#strong[注意：]梯度方向是#strong[灰度增长最快的方向]，而边缘方向与梯度方向垂直。

#strong[常见的一阶微分算子模板（考试必背）：]
...
#strong[要点：]Sobel 算子中的 "2" 用于增强中心像素的重要性。

== 3. 二阶微分算子（拉普拉斯算子）

- #strong[定义：]$nabla^2 f = (partial^2 f)/(partial x^2) + (partial^2 f)/(partial y^2)$。
- #strong[二阶导数的特点：]
  - 对#strong[灰度阶跃]产生#strong[双线响应]（正-负）。
...
#formula[
  若中心为负的模板：$ g(x, y) = f(x, y) - nabla^2 f(x, y) $

  若中心为正的模板：$ g(x, y) = f(x, y) + nabla^2 f(x, y) $
]

== 4. 高频提升与钝化掩模（Unsharp Masking）
...
// ================================================================
// 十一、Lecture 03 核心公式/模板速查表
// ================================================================

= 十一、Lecture 03 核心公式/模板速查
...
#formula[$ E(u, v) approx sum_((x, y) in W) (I_x u + I_y v)^2 $]
...
- #strong[矩阵 $H$]（称为 Harris 矩阵/二阶矩矩阵）描述了窗口内梯度的分布情况。它是一个对称半正定矩阵。
...
+ 步骤 1：#strong[计算梯度]：对图像每个像素，计算 $x, y$ 方向的梯度 $I_x, I_y$。
+ 步骤 2：#strong[构建矩阵]：在每一像素的邻域窗口 $W$（如 $3 times 3$）内，累加计算矩阵 $H$ 的四个元素：$I_x^2, I_y^2, I_x I_y$。
+ 步骤 3：#strong[计算响应值]：根据公式计算每个像素的角点响应函数 $R$。
+ 步骤 4：#strong[阈值处理]：设定阈值 $T$，只保留 $R > T$ 的点（过滤平坦区域和弱边缘）。
+ 步骤 5：#strong[非极大值抑制（NMS）]：在局部邻域内（如 $3 times 3$），只保留 $R$ 值最大（局部极值）的点作为最终角点（防止角点成堆出现）。
...
#strong[1. LeNet-5 (1998)：]Yann LeCun 提出，用于手写数字识别（MNIST）。结构：输入(32×32) → 卷积 → 池化 → 卷积 → 池化 → 全连接 → 输出。奠定了现代 CNN 的基础结构。
...
// ================================================================
// 三十五、Transformer 向 CNN 回归
// ================================================================

= 三十五、Transformer 向 CNN 回归（ConvNeXt & RepLKNet）
...
== 10. 增量学习 / 持续学习（Continual Learning）
...
== 12. 联邦学习（Federated Learning / FedAvg）
...
// ================================================================
// 三十八、Lecture 06_2 核心速查
// ================================================================

= 三十八、Lecture 06_2 核心速查（开卷考试直接抄用）
...
= 四十三、实例分割与目标检测演进（R-CNN → Mask R-CNN）
...
== 1. YOLO（You Only Look Once）核心思想
...
== 3. Focal Loss（RetinaNet, ICCV 2017）
...
= 五十三、底层视觉新任务：去恶劣天气、去反光、人脸/文本恢复

== 1. 去雨/雪/雾（De-weathering）
...
= 八十一、GAN 反演（GAN Inversion）与图像编辑
...
== 3. In-domain GAN Inversion（ECCV 2020）
...
// ================================================================
// 八十九、Lecture 生成模型核心速查
// ================================================================

= 八十九、生成模型核心速查（开卷考试直接抄用）
